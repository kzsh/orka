...
Presets you want in every session — access to an issue tracker, for example — can be listed under `preset` in `~/.config/orka/config.yaml` instead:
...
`orka --preset bun` then applies `jira` and `bun`. Repeating an always-on preset on the command line has no additional effect. A name listed there that does not exist in `environments.yaml` fails the run and the error identifies `config.yaml` as the source.
